...
func TestGenerateCommand_Run_DefaultFlags(t *testing.T) {
	...
	assert.Contains(t, content, "_globalContextDecorator")
	assert.Contains(t, content, "_globalSpanOpts")
	assert.Contains(t, content, "SetDefaultContextDecorator")
	assert.Contains(t, content, "SetDefaultSpanOptions")
	// Shared tracing option types
	assert.Contains(t, content, "TracingOption")
	assert.Contains(t, content, "tracingConfig")
	assert.Contains(t, content, "WithSpanDecorator")
	assert.Contains(t, content, "WithContextDecorator")
	assert.Contains(t, content, "WithSpanOptions")
	// Per-interface decorator
	assert.Contains(t, content, "CommandWithTracing")
	assert.Contains(t, content, "NewCommandWithTracing")
	// Constructor merges global span options
	assert.Contains(t, content, "_globalSpanOpts")
