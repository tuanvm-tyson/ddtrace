...
func Test_unquote(t *testing.T) {
	tests := []struct {
		...
		{name: "unquoted string", s: "abcde", want1: "abcde"},
		{name: "left quote only", s: `"abcde`, want1: "abcde"},
		{name: "right quote only", s: `abcde"`, want1: "abcde"},
		{name: "left and right quotes", s: `"abcde"`, want1: "abcde"},
	}
...
func Test_mergeMethods(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil method", wantErr: false},
		{
			name: "duplicate methods should return outer method",
			args: args{
				ml1: methodsList{"method": Method{Doc: []string{"outer"}}},
				ml2: methodsList{"method": Method{Doc: []string{"inner"}}},
			},
			wantErr: false,
			want1:   methodsList{"method": {Doc: []string{"outer"}}},
		},
		{
			name: "success",
			args: args{
				ml1: methodsList{"method1": Method{}},
				ml2: methodsList{"method2": Method{}},
			},
			want1: methodsList{"method1": Method{}, "method2": Method{}},
		},
	}
...
func Test_processSelector(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: targetProcessInput{
				processInput: processInput{currentPackage: &packages.Package{Imports: nil}},
			},
			wantErr: true,
		},
		{
		...
		},
		{
			...
			input: targetProcessInput{
				processInput: processInput{
					...
					currentPackage: &packages.Package{Imports: map[string]*packages.Package{"io": {}}},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got1, err := processSelector(tt.se, tt.input)
...
func Test_processInterface(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			targetInput: targetProcessInput{
				processInput: processInput{fileSet: token.NewFileSet()},
			},
		...
		},
		{
			...
			it: &ast.InterfaceType{Methods: &ast.FieldList{List: []*ast.Field{
				{Names: []*ast.Ident{{Name: "methodName"}}, Type: &ast.SelectorExpr{X: &ast.Ident{Name: "unknown"}, Sel: &ast.Ident{Name: "Interface"}}},
			}}},
			targetInput: targetProcessInput{
				processInput: processInput{fileSet: token.NewFileSet(), currentPackage: &packages.Package{Imports: nil}},
			},
			wantErr: true,
		},
		{
		...
		},
		{
			...
			it: &ast.InterfaceType{Methods: &ast.FieldList{List: []*ast.Field{
				{Type: &ast.IndexListExpr{X: &ast.Ident{Name: "Embedded"}, Indices: []ast.Expr{&ast.Ident{Name: "Embedded_2"}}}},
			}}},
			targetInput: targetProcessInput{
				processInput: processInput{fileSet: token.NewFileSet()},
				types:        []*ast.TypeSpec{{Name: &ast.Ident{Name: "Embedded"}, Type: &ast.InterfaceType{}}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got1, err := processInterface(tt.it, tt.targetInput)
...
func Test_typeSpecs(t *testing.T) {
	expected := []*ast.TypeSpec{{Name: &ast.Ident{Name: "Interface"}, Type: &ast.InterfaceType{}}}
...
func Test_findTarget(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "not found",
			input:   processInput{astPackage: &scanner.Package{}},
			wantErr: true,
			inspectErr: func(err error, t *testing.T) {
		...
		},
		{
			name: "found",
			input: processInput{
				astPackage: &scanner.Package{Files: map[string]*ast.File{
					"file.go": {Decls: []ast.Decl{&ast.GenDecl{Tok: token.TYPE, Specs: []ast.Spec{&ast.TypeSpec{Name: &ast.Ident{Name: "Interface"}, Type: &ast.InterfaceType{}}}}}},
				}},
			...
			input: processInput{
				astPackage: &scanner.Package{
					Files: map[string]*ast.File{
						"file.go": {Decls: []ast.Decl{&ast.GenDecl{Tok: token.TYPE, Specs: []ast.Spec{&ast.TypeSpec{
							Name: &ast.Ident{Name: "InterfaceAlias"},
							Type: &ast.Ident{
								Name: "Interface",
								//nolint:staticcheck
								Obj: &ast.Object{Decl: &ast.TypeSpec{Type: &ast.InterfaceType{}}},
							},
						}}}}},
					},
				},
		...
		},
		{
					...
					Files: map[string]*ast.File{
						"file.go": {
							Decls: []ast.Decl{&ast.GenDecl{Tok: token.TYPE, Specs: []ast.Spec{&ast.TypeSpec{
								Name: &ast.Ident{Name: "InterfaceAlias"},
								Type: &ast.SelectorExpr{X: &ast.Ident{Name: "io"}, Sel: &ast.Ident{Name: "Reader"}},
							}}}},
							Imports: []*ast.ImportSpec{{Path: &ast.BasicLit{Value: "io"}}},
						},
					},
			...
			want1: methodsList{
				"Read": Method{
					Name:   "Read",
					Params: ParamsSlice{{Name: "p", Type: "[]byte"}},
					Results: ParamsSlice{
						{Name: "n", Type: "int"},
						{Name: "err", Type: "error"},
					},
				...
				astPackage: &scanner.Package{
					Files: map[string]*ast.File{
						"file.go": {
							Decls: []ast.Decl{&ast.GenDecl{Tok: token.TYPE, Specs: []ast.Spec{&ast.TypeSpec{
								Name: &ast.Ident{Name: "InterfaceAlias"},
								Type: &ast.SelectorExpr{X: &ast.Ident{Name: "io_name"}, Sel: &ast.Ident{Name: "Reader"}},
							}}}},
							Imports: []*ast.ImportSpec{{Name: &ast.Ident{Name: "io_name"}, Path: &ast.BasicLit{Value: "io"}}},
						},
					},
				},
				targetName: "InterfaceAlias",
...
func TestGenerator_Generate(t *testing.T) {
	tests := []struct {
		...
		inspect    func(r Generator, w io.Writer, t *testing.T)
		args       func(t minimock.Tester) io.Writer
		wantErr    bool
		inspectErr func(err error, t *testing.T)
	}{
		{
			...
			args:    func(t minimock.Tester) io.Writer { return nil },
			wantErr: true,
		},
		{
		...
		},
		{
			...
			args:    func(t minimock.Tester) io.Writer { return bytes.NewBuffer([]byte{}) },
			wantErr: false,
		},
		{
...
`, w.(*bytes.Buffer).String())
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := receiver.Generate(w)

			if tt.inspect != nil {
				tt.inspect(receiver, w, t)
			}
...
func TestNewGenerator(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			options: func(t minimock.Tester) Options { return Options{HeaderTemplate: "{{."} },
			wantErr: true,
		},
		{
			name:    "bad body template",
			options: func(t minimock.Tester) Options { return Options{HeaderTemplate: "", BodyTemplate: "{{."} },
			wantErr: true,
		},
		{
		...
		},
		{
			...
			options: func(t minimock.Tester) Options {
				return Options{HeaderTemplate: "", BodyTemplate: "", SourcePackage: "./", OutputFile: "not-exist/out.go"}
			},
			wantErr: true,
		},
		{
			...
			options: func(t minimock.Tester) Options {
				return Options{HeaderTemplate: "", BodyTemplate: "", SourcePackage: "./", OutputFile: "./out.go", InterfaceName: "NotExist"}
			},
			wantErr: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got1, err := NewGenerator(tt.options(mc))
	...
	t.Run("unexported interface", func(t *testing.T) {
		options := Options{HeaderTemplate: "", BodyTemplate: "", SourcePackage: "testing", OutputFile: "./out.go", InterfaceName: "TB"}
	...
	t.Run("success", func(t *testing.T) {
		options := Options{HeaderTemplate: "", BodyTemplate: "", SourcePackage: "io", OutputFile: "./out.go", InterfaceName: "Closer"}
		g, err := NewGenerator(options)
		assert.NoError(t, err)
		assert.NotNil(t, g)
	})
